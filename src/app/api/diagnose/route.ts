...
export async function POST(request: NextRequest) {
  ...
  const prompt = `あなたはClaudeCodeを使った自社ツール開発の専門家です。
以下の個人事業主の情報を元に、その人が「持てる自社ツール」の診断結果をJSON形式で生成してください。
...
今のビジネスでのもどかしさ：${body.scaleBarrier}

【診断の方針】
- これまでSaaSを買うか外注するしかなかったものを「自分専用のツール」として持てるという視点で提案する
- 社内ツール（業務効率化・自動化）か外部向けツール（フォロワー・クライアントが使う）か、最も効果が高い方向で提案する
- 入力者がエンジニアでなくても伝わる平易な言葉で説明する
- 具体的なツール名をつける（例：「インスタインサイト分析ツール」「節税余地診断ツール」）
...
{
  "mvpName": "自社ツールの具体的な名称（例：インスタインサイト分析ツール）",
  "mvpDescription": "ツールの具体的な説明。何を入力すると何が返ってくるか、または何ができるか（2文以内）",
  "mvpType": "A/B/C/Dのいずれか1文字",
  "traditionalCost": "SaaSを買い続けた場合の年間費用、または外注した場合の見積もり（例：年間約60万円 / 外注約300万円）",
  "traditionalDuration": "SaaSでの妥協期間、または外注の開発期間（例：ずっと妥協 / 外注4〜6ヶ月）",
  "expectedImpact": "このツールが手元に届いた後の具体的なビジネスインパクト（1文）",
